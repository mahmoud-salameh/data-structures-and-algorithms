...
function transformToLis(obj){
  // Solution code here...
  return Object.entries(obj).map(listObj=>`<li>${listObj.join(': ')}</li>`);
}
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(values=> values.filter(val=>(typeof val === 'number' && val % 5 ===0)).map(value=> Math.pow(2,value)));
  // console.log(typeof);
};
